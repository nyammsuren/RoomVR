...
export function createRoom1(scene, camera, renderer) {
    ...
    hud.innerHTML = `
        ...
        <div class="rule" style="font-size:11px;color:#aaa;margin-top:4px;">
            <b style="color:#4ade80">Straight:</b> PC↔Switch, Router↔Switch<br>
            <b style="color:#f97316">Crossover:</b> PC↔PC, Switch↔Switch
        </div>
        <hr style="border-color:rgba(255,255,255,.15);margin:6px 0">
        <small>[P] Router ON/OFF &nbsp; [R] Арилгах<br>
        [1] Straight &nbsp; [2] Crossover<br>[L] Гэрэл</small>`;
    ...
    lightPanel.innerHTML = `
        <button id="r1lightBtn1" style="background:#ffaa00;color:black;border:2px solid #fff;padding:8px 14px;border-radius:8px;font-size:12px;cursor:pointer;width:120px;text-align:center;" onclick="window.r1toggleLight(1)">💡 Гэрэл 1</button>
        <button id="r1lightBtn2" style="background:#ffaa00;color:black;border:2px solid #fff;padding:8px 14px;border-radius:8px;font-size:12px;cursor:pointer;width:120px;text-align:center;" onclick="window.r1toggleLight(2)">💡 Гэрэл 2</button>`;
    ...
    // UI элементүүдийг харуулах/нуух
    function showUI(show) {
        ...
        cablePanel.style.display = d;
        connPanel.style.display = d;
        lightPanel.style.display = d;
    ...
    loader.load('./lab4.glb',gltf=>{
        ...
        function loadTex(url,rx,ry){
            return textureLoader.load(url,tex=>{tex.colorSpace=THREE.SRGBColorSpace;tex.wrapS=THREE.RepeatWrapping;tex.wrapT=THREE.RepeatWrapping;tex.repeat.set(rx,ry);tex.needsUpdate=true;});
        }
        ...
        applyTex(deever,texWall,0.9,0.0); applyTex(hana1,texWall,0.9,0.0);
        applyTex(hana2,texWall,0.9,0.0); applyTex(hana3,texWall,0.9,0.0);
        ...
        applyColor(shiree1,new THREE.Color(0xD2B48C));
        ...
        const wifiGeo=new THREE.BufferGeometry().setFromPoints(wifiPts);
        wifiLineMesh=new THREE.Line(wifiGeo,new THREE.LineDashedMaterial({color:0x00d4ff,dashSize:0.08,gapSize:0.05,transparent:true,opacity:0.8}));
    ...
    // visible өөрчлөгдөхөд UI харуулах/нуух
    Object.defineProperty(room, 'visible', {
        get() { return this._visible ?? true; },
        set(v) {
            this._visible = v;
            this.children.forEach(c => c.visible = v);
            showUI(v);
        }
    });
